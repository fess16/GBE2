  ...
  $("#bkmk-tree").fancytree({
	  ...
	  activate: function(event, data) {
	  	if (bg.GBE2.opt.enableTreePersisitData) {
		  	...
		  	// при активации закладки (клик левый/правый...) запоминаем ее метку
		  		...
		  		_appendPersist(node.isTopLevel() ? parent.getFirstChild().key : parent.key, true, _TREE_PERSIST_DATA.ACTIVE);
		  	}
		  }
	  },
    ...
    dnd: {
      ...
      dragStart: function(node, data) {
      	...
      	const parent = data.node.getParent();
      	// источник перемещения (текущий родитель или null для верхнего уровня)
      	dragInfo.source = (data.node.isTopLevel()) ? null : parent;
      ...
      dragEnter: function(node, data) {
        ...
        if (!node.isTopLevel() || (parent.key == otherParent.key) || (bg.GBE2.opt.enableLabelUnlabeled && !data.otherNode.isFolder()))
...
*/				
      ...
      dragDrop: function(node, data) {
        ...
        if ((data.hitMode == "before" || data.hitMode == "after") && node.isTopLevel())
